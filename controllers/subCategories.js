...
exports.getSubCategory = async (req, res) => {
    ...
    subCategory ? (res.status(200).json(subCategory)) : (res.status(404).json({ msg: 'subcategory not found' }))
...
exports.createSubCategory = (req, res) => {
    ...
    if (!files) (undefinedError.err = true, undefinedError.name = 'you must to add image')
    if (!sections_id) (undefinedError.err = true, undefinedError.name = 'you must to add section')
    ...
    subCategory ? (res.status(200).json({ msg: 'subcategory created' })) : (res.status(500));
}
